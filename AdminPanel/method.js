    ...
    fetch("http://172.16.102.69:8890/SushiSystem/getTavoli").then((res) => {
        ...
        element.forEach((el) => {
            ...
            el.addEventListener("click", () => {
                if (!stato) {
                    tempEventId = el.id;
                    var wrapperOrdini = document.getElementById("wrapperOrdini");
                    wrapperOrdini.setAttribute("name",el.id);
                    getOrdine(el.id);
        ...
        btnX.forEach((obj) => {
            obj.addEventListener("click", () => {
                ...
                parentDiv3.remove();
                var wrapperOrdini = document.getElementById("wrapperOrdini");
                nTav = wrapperOrdini.getAttribute("name");
                removeOrdine(nTav, parentDiv3.id);
            })
        })
    })
...
function getOrdine(id) {
    ...
    fetch("http://172.16.102.69:8890/SushiSystem/getOrdine", {
        method: 'POST', // or 'PUT'
        headers: {
        ...
        .then(data => {
            ...
            if (data.ordine.ordinazione.length > 0 && data.ordine.quantita.length > 0) {
                for (let i = 0; i < data.ordine.ordinazione.length; i++) {
                    ...
                    if (data.ordine.ordinazione[i] == 50) {
                        setAcquaNat(data.ordine.quantita[i]);
                    } else if (data.ordine.ordinazione[i] == 51) {
                        setAcquaFriz(data.ordine.quantita[i]);
                    } else {
...
function removeOrdine(nTav, idProdotto){
    const data = {"nTavolo": nTav, "idProdotto": idProdotto};
    fetch("http://172.16.102.69:8890/SushiSystem/removeOrdine", {
        method: 'POST', // or 'PUT'
        headers: {
            'Content-Type': 'application/json',
        },
